...
-(NSString *)formattedTime{
    ...
    if (!hasAMPM) { //24小时制
        ...
        if (hour >= 0 && hour <= 6) {
            ...
            dateFormatter = [NSDateFormatter dateFormatterWithFormat:@"yyyy-MM-dd HH"];
        }
...
+ (NSString *)compareTime:(NSDate *)date
{
    ...
    BSTCompareTimeResult result = [NSDate resultForCompare:date];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    
    if(result == compareResultSameDay){//the same day
        [dateFormatter setDateFormat:NSLocalizedString(@"IDS_HH_MM",nil)];
    }
    else if(result == compareResultDifferentYear){//different year
        [dateFormatter setDateFormat:NSLocalizedString(@"IDS_MM_DD_YY_HH_MM", nil)];
    }
    else{//different day
        [dateFormatter setDateFormat:NSLocalizedString(@"IDS_MM_DD_HH_MM",nil)];
    }
    NSString *resultTimeStr = [dateFormatter stringFromDate:date];
...
+ (NSString *)compareDay:(NSDate *)date
{
    ...
    if(result == compareResultSameDay){//the same day
        resultTimeStr = NSLocalizedString(@"IDS_TODAY", nil);
    }
    else{
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        if(result == compareResultDifferentYear){//different year
            [dateFormatter setDateFormat:NSLocalizedString(@"IDS_MM_DD_YY", nil)];
        }
        else{
            [dateFormatter setDateFormat:NSLocalizedString(@"IDS_MM_DD",nil)];
        }
        resultTimeStr = [dateFormatter stringFromDate:date];
    }
    return resultTimeStr;
}
...
+ (BSTCompareTimeResult)resultForCompare:(NSDate *)date
{
    BSTCompareTimeResult result;
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    NSDate *todayTimeDate = [NSDate date];
    
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    todayTimeDate = [dateFormatter dateFromString:[dateFormatter stringFromDate:todayTimeDate]];
    NSDate *startDate = [dateFormatter dateFromString:[dateFormatter stringFromDate:date]];
    
    NSCalendar* chineseClendar = [[ NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSUInteger unitFlags = NSCalendarUnitDay | NSCalendarUnitMonth | NSCalendarUnitYear;
    
    float timeInterval = [todayTimeDate timeIntervalSinceDate:startDate];
    int days=((int)timeInterval)/(3600*24);
    
    if(days == 0){
        result = compareResultSameDay;
    }
    else{
        NSDateComponents *todayCps = [chineseClendar components:unitFlags fromDate:todayTimeDate];
        NSDateComponents *startCps = [chineseClendar components:unitFlags fromDate:startDate];
        
        if(([todayCps year] - [startCps year]) != 0){
            result = compareResultDifferentYear;
        }
        else{
            result = compareResultSameYear;
        }
    }
    return result;
}
